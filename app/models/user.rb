class User < ActiveRecord::Base
  ...
  def self.top_rated
    self.select('users.*'). # Select all attributes of the user
        ...
        joins(:posts). # Ties the posts table to teh users table, via the usser_id
        ...
        order('rank DESC') # Instructs the database to order the results in decending order, by the rank that we created in this query. (rank = comment count + post count)
  end
